...
// 拥塞控制相关的常量
#define INIT_CWND 1
#define INIT_SSTHRESH 32

// 拥塞控制相关变量
enum CongestionState { SLOW_START, CONGESTION_AVOIDANCE, FAST_RECOVERY };
CongestionState congestion_state = SLOW_START;
int cwnd = INIT_CWND;           // 拥塞窗口大小
int ssthresh = INIT_SSTHRESH;   // 慢启动阈值
int duplicate_acks = 0;         // 重复ACK计数
int last_ack = 0;              // 上一个收到的ACK
float cwnd_increment = 0.0;     // 用于拥塞避免状态下的cwnd增量
...
DWORD WINAPI recvackthread(PVOID useparameter) {
	...
	while (1) {
		...
		if (recvByte > 0 && recvMsg.check()) {
			...
			if (recvMsg.AckNum > last_ack) {
				// 新的ACK
				duplicate_acks = 0;
				last_ack = recvMsg.AckNum;
				...
				if (recvMsg.AckNum >= start) {
					...
					cout << Get_Time() << "   [Congestion] State: " <<
						(congestion_state == SLOW_START ? "SLOW_START" :
							congestion_state == CONGESTION_AVOIDANCE ? "CONGESTION_AVOIDANCE" :
							"FAST_RECOVERY") <<
						", cwnd = " << cwnd << ", ssthresh = " << ssthresh << endl;
				}
			}
			else if (recvMsg.AckNum == last_ack) {
				// 重复ACK
				duplicate_acks++;
				if (duplicate_acks == 3) {
					// 进入快速恢复
...
int main() {
	...
	int count = 0;
	while (1) {
		int effective_window = min(Window_Size, cwnd);
		...
		if (arrive < start + Window_Size && arrive < nummessage + 2) {
			...
			cout << Get_Time() << "   " << "[Window] Window size: " << Window_Size - (arrive - start) << ", has sent but not received confirmation: " << (arrive - start) << endl;
			ReleaseMutex(mutex); // 释放互斥锁

		}

		// 超时处理
		if (clock() - messagestart > MAX_WAIT_TIME) {
			WaitForSingleObject(mutex, INFINITE);
			...
			for (int i = 0; i < arrive - start; i++) {
				...
				sendto(clientSocket, (char*)&resendMsg, sizeof(resendMsg), 0,
					(sockaddr*)&serverAddr, sizeof(SOCKADDR_IN));
				cout << Get_Time() << "   [Timeout Resend] seq = " << resendMsg.SeqNum << endl;
